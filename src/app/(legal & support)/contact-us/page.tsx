    ...
    <section className="bg-[var(--bg-light)] pt-20 pb-12 min-h-[70vh]">
      <div className="max-w-8xl  mx-auto px-4 sm:px-8 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-[var(--text-semi-dark)] max-w-2xl mx-auto">{CONTACT_US_TEXT.SUBTITLE}</p>
        </div>
          ...
          {!submitted ? (
            <>
              ...
              <div className="md:w-1/2 flex flex-col items-center justify-start bg-white border-b md:border-b-0 md:border-r border-[var(--base)]/10 p-4 md:p-8">
                <img src="/assets/contact_us.svg" alt="Contact Us Illustration" className="w-48 h-48 md:w-80 md:h-80 object-contain mb-6 md:mb-12" />
                ...
                <div className="w-full mb-6 md:mb-12 flex flex-col md:flex-row gap-4">
                  <div className="bg-white rounded-xl border p-6 flex items-center space-x-3 md:space-x-4 w-full md:w-1/2">
                    <div className="flex-shrink-0 w-5 h-5 md:w-6 md:h-6">
                      <IconMap name="phone" className="w-5 h-5 md:w-6 md:h-6 text-[var(--base)]" />
                    </div>
                    <div>
                      <p className="text-xs md:text-sm font-medium text-[var(--text-dark)]">Phone</p>
                      <p className="text-xs md:text-sm text-[var(--text-semi-dark)]">{CONTACT_US_TEXT.PHONE}</p>
                    </div>
                  </div>
                  <div className="bg-white rounded-xl border p-6 flex items-center space-x-3 md:space-x-4 w-full md:w-1/2">
                    <div className="flex-shrink-0 w-5 h-5 md:w-6 md:h-6">
                      <IconMap name="address" className="w-5 h-5 md:w-6 md:h-6 text-[var(--base)]" />
                    </div>
                    <div>
                      <p className="text-xs md:text-sm font-medium text-[var(--text-dark)]">Address</p>
                      <p className="text-xs md:text-sm text-[var(--text-semi-dark)] whitespace-pre-line">{CONTACT_US_TEXT.ADDRESS}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="w-full">
                  ...
                  <div className="flex justify-center space-x-4 md:space-x-6">
                    {FOOTER_SOCIAL_LINKS.map((social, index) => (
                      ...
                      >
                        <IconMap name={social.icon} className="w-5 h-5 md:w-6 md:h-6 text-[var(--base)]" />
                      </a>
                    ))}
                  </div>
                </div>
              ...
              <div className="md:w-1/2 w-full p-6 md:p-12 flex flex-col justify-center">
                {false ? (
                  <div className="flex flex-col items-center justify-center h-full w-full min-h-[340px]">
                    ...
                    <h3 className="text-2xl font-bold mb-2 mt-4 text-[var(--text-dark)]">{CONTACT_US_TEXT.THANK_YOU}</h3>
                    <p className="text-[var(--text-semi-dark)] mb-6">{CONTACT_US_TEXT.GET_BACK_SOON}</p>
                    <button
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6 contact-us-form">
                ...
                <div className="mt-10 border-t border-[var(--base)]/10 pt-8 text-center">
                  <h4 className="text-lg font-semibold mb-2 text-[var(--text-dark)]">{CONTACT_US_TEXT.OTHER_WAYS}</h4>
                  <p className="text-[var(--text-semi-dark)] mb-2">
                    Email:{" "}
                    <a href="mailto:support@campusgig.com" className="text-[var(--base)] hover:underline">
                      {CONTACT_US_TEXT.EMAIL}
                    </a>
                  </p>
                  <p className="text-[var(--text-semi-dark)]">{CONTACT_US_TEXT.NOTE}</p>
                </div>
              </div>
            </>
          ) : (
