        ...
        <div className="bg-[var(--card-light)] rounded-xl shadow-md p-0 md:p-0 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="md:w-1/2 flex flex-col items-center justify-start bg-white border-b md:border-b-0 md:border-r border-[var(--base)]/10 p-4 md:p-8">
            ...
            <div className="w-full text-center">
              ...
              <p className="text-xs md:text-sm text-[var(--text-semi-dark)] mb-4">
               {FAQ_TEXT.HELP}
              </p>
              <Link href={`/ContactUs`} className="block mx-auto w-fit bg-[var(--base)] text-[var(--text-light)] px-4 py-2 md:px-6 md:py-3 rounded-lg font-semibold hover:bg-[var(--base-hover)] transition-colors duration-300 transform hover:scale-105 text-sm md:text-base">
                Contact Support
              </Link>
            </div>
          </div>
            ...
            <div className="max-h-[600px] overflow-y-auto pr-2 md:pr-4">
              {faqs.length === 0 && !loading ? (
                ...
                faqs.map((faq, index) => (
                  <div
                    key={index}
                    className="mb-4 bg-white rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 border border-[var(--base)]/10"
                  >
                    {/* FAQ Header */}
                    <button
                      onClick={() => toggleFaq(index)}
                      className="w-full p-4 md:p-6 text-left flex items-center justify-between hover:bg-[var(--base)]/5 transition-colors duration-300 group"
                    >
                      ...
                      <div className="flex-shrink-0 ml-2 md:ml-4">
                       <IconMap name='expand' className={`w-5 h-5 md:w-6 md:h-6 text-[var(--text-semi-dark)] transition-transform duration-300 ${
                    ...
                    >
                      <div className="px-4 md:px-6 pb-4 md:pb-6">
                        <div className="border-l-4 border-[var(--base)]/20 pl-3 md:pl-4">
                          <p className="text-xs md:text-sm text-[var(--text-semi-dark)] leading-relaxed">
                            {faq.answer}
                          </p>
                        </div>
                      </div>
