...
import { getInitials } from "../../profile/helper";
...
const ProviderPortfolioPage = () => {
  ...
  const id =
    typeof params.id === "string"
      ...
      ? params.id[0]
      : "";
  const { data, isLoading, error } = useGetProviderPublicProfileQuery(id, {
    skip: !id,
  ...
  const gigs_data = data?.data?.gigs_provider;

  const dataa = gigs_data?.map((item: any) => ({
    tire: item?.gig_category?.tire.name,
    category: item?.gig_category?.name,
  ...
  dataa?.forEach((item: any) => {
    const key = item?.tire;
    if (!grouped.has(key)) {
      grouped.set(key, { tire: key, totalRating: 0, count: 0 });
    }

    const group = grouped.get(key);
    group.totalRating += item?.rating;
    group.count += 1;
  });

  const result = Array.from(grouped.values()).map((group) => ({
    tire: group.tire,
    averageRating: parseFloat((group.totalRating / group.count).toFixed(2)),
    totalReviews: group.count,
  }));

  const badgeValidation = getProviderBadge(
    provider?.isMostRated,
    provider?.isTopRated,
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <div className="flex flex-col gap-3 mt-2 text-sm text-gray-600">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {provider?.skills?.map((skill: any, id: number) => (
                      <span
                        key={id}
                        className="bg-gray-100 rounded px-2 py-1 font-medium"
                      >
                        {skill?.name}
                      </span>
                    ))}
                  ...
                  <div className="flex flx-wrap gap-3">
                    <span className="bg-gray-100 rounded px-2 py-1 font-medium">
                      Completed Projects:{" "}
                      {provider?.gigs_provider?.length > 40
                        ? "40+"
                        : provider?.gigs_provider?.length}
                    </span>
                    <span className="bg-gray-100 rounded px-2 py-1 font-medium">
                      Verified
                    </span>
              ...
              <div className="mt-6 sm:mt-0 flex flex-col items-center bg-[var(--base)]/10 rounded-xl px-8 py-4 min-w-[160px] shadow border border-[var(--base)]/20">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-3xl font-bold text-[var(--base)]">
                    {getAverageRating(
                      provider?.gigs_provider?.map(
                ...
                <div className="text-gray-500 text-xs">
                  {provider?.gigs_provider?.length || 0} reviews
                </div>
              </div>
            </div>
          </div>
        ...
        {result?.length > 0 && <div className="max-w-8xl mx-auto mt-12 mb-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {result?.map((item) => (
              <div
                key={item.tire}
                className="border border-[var(--base)]/10 bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 hover:scale-[1.02] flex flex-col items-center text-center space-y-3"
              >
                <div className="text-xl font-bold text-[var(--base)]">
                  {item.tire}
                </div>
