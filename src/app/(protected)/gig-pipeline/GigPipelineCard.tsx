...
import { Calendar, DollarSign, MapPin, Clock, Star, User, TrendingUp } from "lucide-react";
...
const GigCard = ({ gig, onStartGig, onCompleteGig, onPriorityChange }: GigCardProps) => {
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              ...
              <Badge className={`${getStatusColor(gig.status)} border font-medium`}>
                {getStatusText(gig.status)}
              </Badge>
              {gig.priority && (
                <Badge className={`${getPriorityColor(gig.priority)} border font-medium`}>
            ...
            <div className="flex items-center gap-4 mb-3">
              ...
              {gig.bidAmount && (
                <div className="flex items-center gap-1 text-sm text-teal-600 bg-teal-50 px-2 py-1 rounded-md">
                  ...
                  <span className="font-medium">Your bid: ${gig.bidAmount.toFixed(2)}</span>
                </div>
              )}
            </div>

            <p className="text-gray-600 text-sm leading-relaxed">{gig.description}</p>
          </div>
        </div>
      </CardHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          ...
          {gig.acceptedDate && (
            <div className="flex items-center gap-2 text-sm text-gray-600 bg-green-50 p-2 rounded-md">
              ...
              <span className="text-green-700 font-medium">Accepted: {gig.acceptedDate}</span>
            </div>
          )}
        </div>
              ...
              <div className="flex gap-1">
                {['high', 'medium', 'low'].map((priority) => (
                  <Button
                    ...
                    className={gig.priority === priority ? "bg-teal-600 hover:bg-teal-700" : ""}
                  >
                    {priority}
                  </Button>
                ))}
        ...
        {gig.status === 'completed' && gig.rating && (
          <div className="border-t pt-4 bg-green-50 rounded-lg p-4 mt-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-semibold text-green-800">Client Review:</span>
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    key={star}
                ...
                <span className="text-sm text-green-700 ml-1 font-medium">({gig.rating}/5)</span>
              </div>
            </div>
            {gig.review && (
              <p className="text-sm text-green-700 italic mb-2">"{gig.review}"</p>
            )}
            {gig.completedDate && (
              <div className="flex items-center gap-2 text-sm text-green-600">
                <Clock className="h-4 w-4" />
                <span className="font-medium">Completed on {gig.completedDate}</span>
              </div>
            )}
          </div>
        )}
        ...
        {gig.status === 'rejected' && (
          <div className="border-t pt-4 bg-red-50 rounded-lg p-4 mt-4">
            <p className="text-sm text-red-700">
              <strong>Not selected:</strong> The client chose another provider for this gig. 
              Keep applying to more opportunities!
            </p>
            {gig.bidDate && (
        ...
        {gig.status === 'requested' && (
          <div className="border-t pt-4 bg-blue-50 rounded-lg p-4 mt-4">
            <p className="text-sm text-blue-700">
              <strong>⏳ Waiting for approval:</strong> Your bid is under review by the client. 
