...
export const SmallLoader = ({ loading }: { loading: boolean }) => {
  return (
    loading && (
      <svg
        aria-hidden="true"
...
export const CentralLoader = ({ loading }: { loading: boolean }) => {
...
const Loader: React.FC<{ size?: number; colorClass?: string }> = ({ size = 24, colorClass = "text-[var(--base)]" }) => (
  <span className={`inline-block animate-spin ${colorClass}`} style={{ width: size, height: size }} role="status" aria-label="Loading">
