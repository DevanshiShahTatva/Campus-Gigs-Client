  ...
  const plans = [
    {
      ...
      buttonStyle: "bg-[var(--base)]/10 text-[var(--base)] hover:bg-[var(--base)] hover:text-white border-2 border-[var(--base)]",
      icon: "🚀"
    },
    {
      name: "Premium",
      ...
      buttonStyle: "bg-[var(--base)] text-white hover:bg-[var(--base-hover)] shadow-lg",
      popular: true,
      icon: "⭐"
    },
    {
      ...
      buttonText: "Go Pro",
      buttonStyle: "bg-gradient-to-r from-[var(--base)] to-[var(--base-hover)] text-white hover:from-[var(--base-hover)] hover:to-[var(--base)] shadow-xl",
      icon: "👑"
    },
  ];
    ...
    <section id="pricing" className="py-12 sm:py-16 lg:py-20 bg-gradient-to-br from-white via-[var(--bg-light)] to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 sm:mb-16">
          <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-[var(--text-dark)]">
            Choose Your Plan
          </h2>
          <p className="text-[var(--text-semi-dark)] max-w-3xl mx-auto text-base sm:text-lg px-4">
            Get access to premium features and maximize your CampusGig experience.
            All plans include our basic features plus exclusive benefits.
          </p>
        </div>
          ...
          {plans.map((plan, index) => (
            <div
              ...
              className={`relative bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-500 transform hover:-translate-y-2 sm:hover:-translate-y-4 hover:shadow-2xl group ${
                plan.popular ? "ring-2 ring-[var(--base)] lg:scale-105" : ""
            ...
            >
              ...
              <div className="relative p-6 sm:p-8 flex flex-col h-full">
                ...
                <div className="text-center mb-4">
                  <span className="text-4xl sm:text-5xl font-bold text-[var(--base)]">
                    {plan.price}
                  </span>
                  {plan.period && (
                    <span className="text-[var(--text-semi-dark)] ml-1 text-base sm:text-lg">{plan.period}</span>
                  )}
                </div>
                ...
                <ul className="space-y-3 sm:space-y-4 mb-6 sm:mb-8 flex-grow">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3 group-hover:translate-x-2 transition-transform duration-300" style={{ transitionDelay: `${i * 100}ms` }}>
                      ...
                      <span className="text-[var(--text-semi-dark)] group-hover:text-[var(--text-dark)] transition-colors duration-300 text-sm sm:text-base">{feature}</span>
                    </li>
                  ))}
                </ul>
                ...
                <Link href="/sign-up">
                  <button
                    className={`w-full py-3 sm:py-4 px-4 sm:px-6 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 mt-auto text-sm sm:text-base ${plan.buttonStyle}`}
        ...
        <div className="text-center">
          <h3 className="text-xl sm:text-2xl font-semibold mb-8 text-[var(--text-dark)]">
            All Plans Include
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto">
            {categories.map((category, index) => (
              ...
              >
                <div className="text-2xl sm:text-3xl mb-4 text-center group-hover:scale-110 transition-transform duration-300">
                  {category.icon}
                </div>
        ...
        <div className="mt-12 sm:mt-16 text-center">
          <div className="bg-gradient-to-r from-[var(--base)]/10 to-[var(--base)]/5 rounded-2xl p-6 sm:p-8 max-w-3xl mx-auto border border-[var(--base)]/20">
            <h3 className="text-xl sm:text-2xl font-semibold mb-4 text-[var(--text-dark)]">
