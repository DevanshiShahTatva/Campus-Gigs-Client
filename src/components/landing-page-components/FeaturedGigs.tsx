  ...
  return (
    <section className="py-20 bg-[var(--bg-dark)]">
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-3xl font-bold text-center mb-12 text-[color:var(--text-light)]">Featured Gigs & Providers</h2>

        {/* Featured Gigs - Responsive Grid */}
        <div className="mb-16">
          <h3 className="text-xl font-semibold mb-6 text-[color:var(--text-light)] text-center">Featured Gigs</h3>
            ...
            {FEATURED_GIGS.map((gig) => (
              ...
              >
                ...
                <div className="relative mb-4 overflow-hidden rounded-lg">
                  <img src={gig.image} alt={gig.title} className="w-full h-32 object-cover transition-transform duration-500 group-hover:scale-110" />
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-sm text-[color:var(--text-light)]/70 mb-3 line-clamp-2">{gig.description}</p>

                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[color:var(--base)] font-bold text-lg">{gig.price}</span>
                    <div className="flex items-center gap-1">
                      <span className="text-yellow-400 text-sm">{renderStars(gig.rating)}</span>
                      <span className="text-[color:var(--text-light)]/70 text-xs">({gig.reviews})</span>
                    </div>
                  </div>

                  <div className="flex items-center justify-between">
                    <span className="text-sm text-[color:var(--text-light)]/60">by {gig.provider}</span>
        ...
        {/* Top Providers */}
        <div>
          <h3 className="text-xl font-semibold mb-6 text-[color:var(--text-light)] text-center">Top Providers</h3>
            ...
            {FEATURED_GIGS_PROVIDERS.map((provider) => (
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-sm text-[color:var(--text-light)]/70 mb-2">{provider.specialty}</p>
                  <p className="text-sm text-[color:var(--base)] mb-3 font-semibold">{provider.tier} Provider</p>

                  <div className="flex items-center justify-center gap-1 mb-4">
                    <span className="text-yellow-400 text-sm">{renderStars(provider.rating)}</span>
                    <span className="text-[color:var(--text-light)]/70 text-xs">({provider.reviews}+)</span>
                  </div>
